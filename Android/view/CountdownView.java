package com.yilin.qucaile.view;
...
/**
 * Created by zhanghaitao on 2018/3/12.
 * 已经经过实战验证。
 * 需要在attrs.xml里面添加：
  <!--倒计时控件 CountdownView-->
    <declare-styleable name="CountdownView">
        <attr name="countDownFormat" format="string" />
        <attr name="countDown" format="float" />
        <attr name="countDownInterval" format="float" />
        <attr name="enableCountDown" format="boolean" />
        <attr name="cdFinishText" format="string" />
    </declare-styleable> 
 */
...
public class CountdownView extends Button {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mCountDownFormat = typedArray.getString(R.styleable.CountdownView_countDownFormat);
        mCdFinishText = typedArray.getString(R.styleable.CountdownView_cdFinishText);
        if (typedArray.hasValue(R.styleable.CountdownView_countDown)) {
            mCount = (int) typedArray.getFloat(R.styleable.CountdownView_countDown, DEFAULT_COUNT);
        }
        mInterval = (int) typedArray.getFloat(R.styleable.CountdownView_countDownInterval, DEFAULT_INTERVAL);
        mEnableCountDown = (mCount > mInterval) && typedArray.getBoolean(R.styleable.CountdownView_enableCountDown, true);
        typedArray.recycle();
        // 初始化倒计时Timer
    ...
    private int stopCode = 666;

    private Handler handler = new Handler() {
        @Override
        public void handleMessage(Message msg) {
            super.handleMessage(msg);
            if (msg.what == stopCode) {
                if (mCountDownTimer != null) {
